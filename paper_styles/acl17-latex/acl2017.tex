...
However, you should maintain space for names and addresses so that they will fit in the final (accepted) version.  The ACL 2017 \LaTeX\ style will create a titlebox space of 6.35 cm for you when {\verb|\aclfinalcopy|} is commented out.  

\subsection{The Ruler}
The ACL 2017 style defines a printed ruler which should be present in the
...
{\bf Reviewers}: note that the ruler measurements do not align well with
...
references ({\em e.g.}, the first paragraph on this page ends at mark $116.5$).
...
{\small\tt acl2017.org/downloads/acl17-latex.zip}.

A Microsoft Word template file ({\small\tt acl17-word.docx}) and example submission pdf ({\small\tt acl17-word.pdf})
is available at
 {\small\tt acl2017.org/downloads/acl17-word.zip}.
...
columns (or, in the case of initial submission, {\bf Anonymous ACL submission} for names and affiliations). Do not use footnotes for affiliations. 
Do include the paper ID number assigned during the submission process in the header. 
Use the
...
{\bf Indent}: Indent when starting a new paragraph, about 0.4 cm.
...
%{\bf Placement}: Place figures and tables in the
...
{\bf Placement}: Place figures and tables in the paper near where they are first discussed, as close as possible to the top of their respective column.
...
The numbering (if any) and alignment of the equations will be done automatically (using \verb|eqnarray| or \verb|equation|).
